    ...
    firebase.database().ref(`school${user.school}/students/student${name.toLowerCase().trim()} ${klass.toLowerCase().trim()}/messagesFrom${username}`).orderByChild('date/minutes').on('child_added', (data) => {
        try {
            if (data.val().voiceId) {
                ...
                storageRef.child(`images/${username}/${imageMsg}`).getDownloadURL().then((url) => {
                    document.getElementById('messages').innerHTML += `
                        <div class="card">
                            <div class="card-body">
...
function chatTeacherAndStudent(username, myName, studentKlass) {
    ...
    document.getElementById('send-image').addEventListener('click', () => {
        ...
        fileInput.addEventListener('change', (e) => {
            ...
            storageRef.child(`images/${username}/${myName}${day}${month}${year}${hour}${minutes}${seconds}`).put(file).then(() => {
                firebase.database().ref(`school${user.school}/students/student${username.toLowerCase().trim()} ${studentKlass.toLowerCase().trim()}/messagesFrom${myName}/${image_id}`).set({
                    fullName: myName,
...
function chatStudentAndTeacher(username, myName, studentKlass) {
    ...
    document.getElementById('send-image').addEventListener('click', () => {
        ...
        fileInput.addEventListener('change', (e) => {
            ...
            storageRef.child(`images/${username}/${myName}${day}${month}${year}${hour}${minutes}${seconds}`).put(file).then(() => {
                firebase.database().ref(`school${user.school}/students/student${myName.toLowerCase().trim()} ${studentKlass.toLowerCase().trim()}/messagesTo${username}/${image_id}`).set({
                    fullName: myName,
                    imageMsg: `${myName}${day}${month}${year}${hour}${minutes}${seconds}`,
                    imageId: image_id,
                    date: {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true})
        .then(stream => {
            ...
            mediaRecorder.addEventListener("dataavailable", (event) => {
                ...
                firebase.database().ref(`school${user.school}/students/student${myName.toLowerCase().trim()} ${studentKlass.toLowerCase().trim()}/messagesTo${username}/${voice_id}`).set({
                    fullName: myName,
                    voiceMsg: `${myName}${day}${month}${year}${hour}${minutes}${seconds}`,
                    voiceId: voice_id,
                    date: {
