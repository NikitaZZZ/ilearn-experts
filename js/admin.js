...
function tests() {
  ...
  firebase
    .database()
    .ref(`school${user.school}/tests/${user.myClass}/`)
    .on('child_added', (data) => {
      ...
      for (let i = 0; i < questions_db.length; i++) {
        ...
        if (questions_db[question_number_admin].option_1 != undefined) {
          ...
          switch (questions_db[question_number_admin].correct_answer) {
            case 1:
              document.getElementById(`option1_text${id}${question_number_admin}`).className =
                'text-success';
              break;
            case 2:
              document.getElementById(`option2_text${id}${question_number_admin}`).className =
                'text-success';
              break;
            case 3:
              document.getElementById(`option3_text${id}${question_number_admin}`).className =
                'text-success';
              break;
            case 4:
              document.getElementById(`option4_text${id}${question_number_admin}`).className =
                'text-success';
              break;
          }
  ...
  firebase
    ...
    .on('value', (snapshot) => {
      for (let key in snapshot.val()) {
        firebase
          .database()
          .ref(`school${user.school}/tests/${user.myClass}/${key}/results`)
          .on('child_added', (data) => {
            ...
            switch (appraisal) {
              case 2:
                document.getElementById(`appraisalDiv${idTest}`).className =
                  'appraisal alert alert-danger';
                break;
              case 3:
                document.getElementById(`appraisalDiv${idTest}`).className =
                  'appraisal alert alert-warning';
                break;
              case 4:
                document.getElementById(`appraisalDiv${idTest}`).className =
                  'appraisal alert alert-primary';
                break;
              case 5:
                document.getElementById(`appraisalDiv${idTest}`).className =
                  'appraisal alert alert-success';
                break;
            }
          });
      }
...
function createQuestion() {
  ...
  if (!cCQ) {
    if (answerOptionsForm[4].checked) {
    } else {
      let file = document.getElementById('files').files[0];
      firebase
        .storage()
        .ref(`/test${idTest}/` + `question${question_number}`)
        .put(file);
    }
    ...
    if (writtenResponseForm[1].checked) {
    } else {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      if (result.isConfirmed) {
        firebase
          .database()
          .ref(`school${user.school}/tests/${user.myClass}/test${idTest}/results`)
          .remove();
