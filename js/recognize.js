...
const phrasesMainPage = [
  {
    ...
    func: () =>
      user.who == 'teacher' ? (location.href = 'itests.html') : (location.href = 'tests.html'),
  },
  {
    possibleUserPhrases: [
      'Открой чаты',
  ...
  },
  {
    ...
    func: () =>
      user.who == 'teacher'
        ? new bootstrap.Modal(document.getElementById('todo')).show()
        : console.log('nooo'),
  },
  {
    ...
    func: () =>
      user.who == 'teacher'
        ? new bootstrap.Offcanvas(document.getElementById('myClass')).show()
        : console.log('nooo'),
  },
  {
    possibleUserPhrases: [
    ...
    func: () =>
      user.who == 'teacher'
        ? new bootstrap.Offcanvas(document.getElementById('mySchool')).show()
        : console.log('nooo'),
  },
  {
    possibleUserPhrases: [
  ...
  },
  {
    possibleUserPhrases: [
      ...
      'число заметок',
      'сколько задач',
      'сколько незаконченных заметок',
    ],
    func: () =>
      user.who == 'teacher'
        ? speak(`Число незаконченных дел: ${ILVue.tasks.length}`)
        : console.log('noooo'),
  },
];
...
try {
  ...
  assistantBtnNonActive.onmousedown = function () {
    ...
    if (!record) {
      ...
      recognition.stop();
    } // Если запись идет, то останавливаем ее
  };
  ...
  if (!localStorage.getItem('hideInfoSR')) {
    Swal.fire({
      ...
      if (result.isDenied) {
        localStorage.setItem('hideInfoSR', true);
      }
    });
  }
