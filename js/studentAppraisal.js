...
firebase
  .database()
  .ref(`school${user.school}/tests/${user.klass}`)
  .on('value', (snapshot) => {
    for (let key in snapshot.val()) {
      firebase
        .database()
        .ref(`school${user.school}/tests/${user.klass}/${key}/results`)
        .on('child_added', (data) => {
...
function createGraphs(subject) {
  try {
    ...
    console.log("chart hasn't been created yet");
  }
  ...
  myChart = new Chart(document.getElementById('graphs'), config);
}
...
function averageScore(subject) {
  ...
  appraisals.forEach((result) => {
    if (result.subject == subject) appraisals_subject.push(result.appraisal);
  });

  appraisals_subject = filterArray(appraisals_subject);
  let averageScoreAppraisals =
    appraisals_subject.reduce((a, b) => {
      return a + b;
    }) / appraisals_subject.length;

  appraisals_subject.forEach((appraisal) => {
    document.querySelector(
      '.all-appraisals',
    ).innerHTML += `<span class="badge bg-primary">${appraisal}</span>`;
  });

  document.querySelector(
    '.average-score',
  ).innerHTML = `<span class="badge bg-success">${averageScoreAppraisals}</span>`;
}
